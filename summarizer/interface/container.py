...
class Container(containers.DeclarativeContainer):
    ...
    dynamodb = providers.Singleton(boto3.resource, 'dynamodb', endpoint_url=config.dynamodb_url)
    feature_repository = providers.Singleton(FeatureRepository, dynamodb)
    video_repository = providers.Singleton(VideoRepository, dynamodb, algorithm=config.detector_model)